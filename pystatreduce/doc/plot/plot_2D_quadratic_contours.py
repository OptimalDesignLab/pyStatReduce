...
scaled_vec = np.matmul(V_orig, quiver_scale) # V_orig # np.matmul(quiver_scale, V_orig)
qX = np.zeros(2)
qY = np.zeros(2)
qU = scaled_vec[0, :]
qV = scaled_vec[1, :]
...
# fname = "2D_quadratic_contour_std_normal.pdf"
fname = "2D_quadratic_contour_diff_sigma.pdf"

plt.rc('text', usetex=True)
matplotlib.rcParams['mathtext.fontset'] = 'cm'
fig = plt.figure("reduced_quadratic", figsize=(6,6))
ax = plt.axes()
cp = ax.contour(xi_1, xi_2, J_xi, levels=[2,4,8,16,32,64,128,256, 512], cmap="coolwarm", linewidths=0.5)
